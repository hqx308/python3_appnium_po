...
import time,os
from config.appconfig import AppRunConfig

class Base(object):
    driver = None
    capabilities = AppRunConfig.capabilities
    ...
    def find_elements(self,loc):
        try:
            #t1=self.driver.find_elements(*loc)
    ...
    #重新封装按钮点击方法，适用有多个元素的情况
    def clickButtons(self,loc,num,find_first=True):
        try:
            if find_first:
                self.find_elements(loc)
            #t1=self.find_elements(loc)
